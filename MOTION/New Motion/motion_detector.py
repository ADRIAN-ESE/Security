...
MIN_RECORD_SECS     = 3       # always record at least this long once triggered
...
def cleanup_old(base_dir, days):
    ...
    for entry in os.listdir(base_dir):
        ...
        try:
            ...
            pass  # skip non-date-named entries

def draw_overlay(frame, status, recording, motion_count, rec_elapsed):
    h, w = frame.shape[:2]
    ...
    # Motion event counter (bottom-left)
...
# Use MOG2 background subtractor for more robust detection
...
while True:
    ...
    # Also keep classic frame-diff as a secondary check (better for slow cameras)
    ...
    # ── Overlay & display ──
    draw_overlay(frame, motion_detected, recording, motion_events, rec_elapsed)
